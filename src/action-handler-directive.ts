...
customElements.define('action-handler-googlehome', ActionHandler);

const getActionHandler = (): ActionHandler => {
  const body = document.body;
  if (body.querySelector('action-handler-googlehome')) {
    return body.querySelector('action-handler-googlehome') as ActionHandler;
  }

  const actionhandler = document.createElement('action-handler-googlehome');
...
export const actionHandler = directive(
  class extends Directive {
    update(part: AttributePart, [options]: DirectiveParameters<this>) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      actionHandlerBind((part as any).element as ActionHandlerElement, options);
      return noChange;
    }
